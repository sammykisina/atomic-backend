...
CONTAINER_PHP=atomic-backend-container
CONTAINER_REDIS=redis
CONTAINER_DATABASE=atomic-backend-db-container
...
get-in: ## Run PHP artisan commands inside the PHP container
	docker exec -it ${CONTAINER_PHP} /bin/bash

pint: ## Run Laravel Pint to check coding standards
	./vendor/bin/pint
